...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (tokenFromUrl) {
      console.log('Admin App: Token found in URL, attempting authentication (optional)...');
      ...
      const fetchUserData = async () => {
        try {
          ...
          if (response.ok) {
            ...
            console.log('Admin App: User data from API:', userData);
            ...
            console.warn('Admin App: Authentication response not OK:', response.status);
            localStorage.removeItem('authToken');
            setToken(null);
          }
          // Clear URL params
          ...
          console.warn('Admin App: Optional authentication failed, continuing without auth:', error);
      ...
      if (storedToken && storedUser) {
        try {
          ...
          console.error('Admin App: Error parsing stored user data:', error);
          localStorage.removeItem('authToken');
          localStorage.removeItem('user');
        }
      }
  ...
  const logout = () => {
    console.log('Admin App: Logging out...');
    setUser(null);
    setToken(null);
    setLoading(false);
    // Clear all storage
