...
const HistoryPage = () => {
  ...
  const applyFilters = () => {
    ...
    if (start || end) {
      filtered = filtered.filter(delivery => {
        const deliveryDate = new Date(delivery.assignedAt || delivery.deliveredAt);
  ...
  const completedDeliveries = displayedHistory.filter(d => d.status === 'Delivered').length;
            ...
            <div className="card-body">
              {loading ? (
                <div className="text-center py-5">
                  ...
                  <p className="mt-3 text-muted">Loading delivery history...</p>
                </div>
              ) : displayedHistory.length === 0 ? (
                    ...
                    <tbody>
                      {displayedHistory.map((delivery) => (
                        <tr key={delivery.deliveryId || delivery.transactionCode}>
                          <td>
                            <strong>{delivery.transactionCode || 'N/A'}</strong>
                          </td>
